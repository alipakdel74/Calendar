package ir.uneed.calender.date.persian
...
object LookupTableConverter {
    ...
    fun toJdn(year: Int, month: Int, day: Int): Long {
        return if (year < startingYear || year > startingYear + yearsStartingJdn.size - 1) -1 else (yearsStartingJdn[year - startingYear]
                + (if (month <= 7) (month - 1) * 31L else (month - 1) * 30L + 6) // total days of months
                + day) - 1
    }
    ...
    fun fromJdn(jdn: Long): IntArray? {
        ...
        val month: Int = if (dayOfYear <= 186) ceil(dayOfYear / 31.0)
            .toInt() else ceil((dayOfYear - 6) / 30.0)
            .toInt()
        val day = dayOfYear.toInt() - if (month <= 7) (month - 1) * 31 else (month - 1) * 30 + 6
